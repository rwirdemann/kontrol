...
func TestErloesverteilungAnEmployees(t *testing.T) {
	...
	net[shrepo.Get("AN").Id] = 100.0
	net[shrepo.Get("JM").Id] = 100.0
	net[shrepo.Get("BW").Id] = 100.0
	...
	*bkgs = append(*bkgs, *booking.NewBooking(13, "ER", "", "", "BW", "Project-X", net, 11.9, "H-costs", 1, 2018, its2018))
	...
	if DEBUG {
		log.Println("in TestStakeholderYearlyIncome: ", len(as.GetCollectiveAccount_thisYear().Bookings))
	}
	...
	for _, sh := range shrepo.All(2018) {
		...
		if DEBUG {
			log.Println("in TestErloesverteilungAnEmployees: ", sh.Id, acc)
		}
	}
	// ==> Ben sollte nun  70€ + 70€ + 5% von 300€ netto€ = 155€ Jahreseinkommen haben...
